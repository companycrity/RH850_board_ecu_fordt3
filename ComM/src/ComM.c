...
/* Check the version of ComM_EcuMBswM.h */
    ...
    || (COMM_ECUMBSWM_MINOR_VERSION != (0u)) \
...
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncStateTransition( ComM_PncIterType ComM_CurrentPn, ComM_PncModeType highestComModeReq )
{
  /* #10 Perform as many state transitions as needed to reach the target requested Pnc mode.
   *     1) Send COM signal if entering or leaving Pnc Requested state,
   *     2) Set the new Pnc state,
   *     3) Notify BswM about the new state,
...
# endif
    ...
    for (ComM_ChannelIndex = 0u; ComM_ChannelIndex < ComM_GetSizeOfChannel(); ComM_ChannelIndex++)
    {
      ...
      if (ComM_GetWakeupStateOfChannel(ComM_ChannelIndex) >= COMM_MAX_NUMBER_OF_STATES) /* COV_COMM_GENDATA_CHECK */ /*lint !e506 */
      {
        errorId = COMM_E_WRONG_GENERATED_DATA;
      }
    }
...
# if (COMM_PNC_SUPPORT)
    if (ComM_GetSizeOfPnc() != ComM_GetSizeOfPncPb())  /* COV_COMM_GENDATA_CHECK */ /*lint !e506 */
    {
      errorId = COMM_E_WRONG_GENERATED_DATA;
    }
# endif

    if (errorId != COMM_E_WRONG_GENERATED_DATA) /* COV_COMM_GENDATA_CHECK */ /* PRQA S 3355, 3358 */ /* MD_ComM_3355_3358 */
#endif
    {
...
# endif
      {
        /* #30 If PNC wake-up is enabled, propagate the wake-up to
         *     - all channels mapped to the PNC and
