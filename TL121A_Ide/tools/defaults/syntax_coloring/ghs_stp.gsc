...
%include "${__TOOLS_DEFAULTS_DIR__}/syntax_coloring/cpp.gsc"
...
language {
	...
	name = "GHS-Transport-Protocol-Script"
	...
	name += {"ASCIIAPI"}
	name += {"COMMANDSYSTEM"}
	name += {"SIDE1STAMP","SIDE1_STAMP","SIDE2STAMP","SIDE2_STAMP"}
	name += {"TMPDEPTH", "TMP_DEPTH"}
	name += {"MESSAGE_DESCRIPTIONS_REQUIRED"}
	# includes section
	name += {"INCLUDES"}
	# constants section
	name += {"CONSTANTS"}
	# types section
	name += {"TYPES"}
	name += {"STRUCT", "CLASS", "UNION", "ENUM"}
	name += {"USERDEFINED"}
	name += {"AS"}
	name += {"PASSBYREF","NOPASSBYREF"}
	name += {"SELFDESTRUCT"}
	name += {"FAST","INLINE"}
	# messages section
	name += {"MESSAGES"}
	name += {"to"}
	name += {"BLOCKING","SYNC","CALL"}
	name += {"NONBLOCKING","ASYNC","LISTEN"}
	name += {"SAFE","UNSAFE"}
	name += {"DELIVERINCOMING","DELIVER_INCOMING","DONT_DELIVER_INCOMING_EXCEPT_SAFE"}
	name += {"OPTIONAL"}
	name += {"MAYRAISEWINDOW","MAY_RAISE_WINDOW","RAISEWINDOW","RAISE_WINDOW"}
	name += {"IN","OUT"}
	name += {"MD"}
	# builtin data types
	name += {"bool"}
	...
	name += {"float","double"}	
	name += {"int8","uint8","int16","uint16"}
	name += {"int32","uint32","int64","uint64"}
	name += {"int","uint"}
	name += {"file_name"}
	name += {"string_list"}
...
%endif
    ...
    # Override c-style preprocessors loaded from cpp.gsc.
    preprocessor {
	name = {""}
    }
}
