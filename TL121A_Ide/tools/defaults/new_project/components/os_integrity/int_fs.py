...
def os_fsmnt_find_mntfile_in_project(defs):
	...
	if (os.access(diskname,os.F_OK)):
	    ret = fs_find_mounttable(defs,[diskname],tell_mprojmgr=False);
	    if (ret != None):
		return ret;
...
def os_fsmnt_tf_mntfile_name_add_std(defs):
    ...
    ret = os_fsmnt_find_mntfile_in_project(defs)
    if (ret != None):
...
def os_fsmnt_int(defs):
    verbose_print(defs,  'os_fsmnt_int()');
    npw2, uid = getNPW2Connection(defs);
...
def os_fsmnt_guifixup(defs, npw2, uid, mnt_name, pd_type,bt_rd):
	verbose_print(defs,  'GUI fixup %s %s %s'  % (mnt_name, pd_type, bt_rd));
		...
		os_fsmnt_do_override(npw2,uid,mnt_name,"bs_nfsopts","on");
                #npw2.OverrideAttribute(uid,"bs_nfsopts","on");
		os_fsmnt_do_override(npw2,uid,mnt_name,"bs_cd9660opts","off");
                #npw2.OverrideAttribute(uid,"bs_cd9660opts","off");
		os_fsmnt_do_override(npw2,uid,mnt_name,"bs_msdosfsopts","off");
                #npw2.OverrideAttribute(uid,"bs_msdosfsopts","off");
		...
		os_fsmnt_do_override(npw2,uid,mnt_name,"bs_msdosfsopts","on");
                #npw2.OverrideAttribute(uid,"bs_msdosfsopts","on");
        

	if (pd_type == "NFS Client" or pd_type == "ISO9660"):
	    os_fsmnt_do_override(npw2,uid,mnt_name,"fsck_opts_available","off");
	else:
	    os_fsmnt_do_override(npw2,uid,mnt_name,"fsck_opts_available","on");
...
def os_fsmnt_comment_std(defs):
   ...
   # num = fs_find_mountpoint_number_by_name(defs,defs["tf_diskname"],defs["LAST_TF_DIR"]);
   # fs_comment_mountpoint_by_number(defs["tf_diskname"],num,defs);
#end os_fsmnt_comment_std

def os_fsmnt_uncomment_std(defs):
    verbose_print(defs,  'os_fsmnt_uncomment_std()');
   # num = fs_find_mountpoint_number_by_name(defs,defs["tf_diskname"],defs["LAST_TF_DIR"]);
   # fs_uncomment_mountpoint_by_number(defs["tf_diskname"],num,defs);
...
#def os_fsmnt_tf_dir_edit_std(defs):
#    print 'os_fsmnt_tf_dir_edit_std()';
#
##    oldstr = defs["LAST_TF_DIR"];
##    newstr = defs["tf_dir"];
##    
##    edit_sub(defs,oldstr,newstr);
#    fs_edit_mountpoint_by_number(defs["tf_diskname"],0,defs);
##end os_fsmnt_tf_dir_edit_std
#
#def os_fsmnt_pd_dev_edit_std(defs):
#    print 'os_fsmnt_pd_dev_edit_std()';
#
##    oldstr = defs["LAST_PD_DEV"];
##    newstr = defs["pd_dev"];
##    
##    edit_sub(defs,oldstr,newstr);
#    fs_edit_mountpoint_by_number(defs["tf_diskname"],0,defs);
##end os_fsmnt_pd_dev_edit_std
#
#def os_fsmnt_tf_slice_edit_std(defs):
#    print 'os_fsmnt_tf_slice_edit_std()';
#
#
#    if (defs["LAST_TF_SLICE"] == "" or defs["LAST_TF_SLICE"] == "All"):
#	oldslicestr = defs["pd_dev"];
#    else:
#	oldslicestr = defs["pd_dev"] + ":" + defs["LAST_TF_SLICE"] 
#	
#    if (defs["tf_slice"] == "" or defs["tf_slice"] == "All"):
#	newslicestr = defs["pd_dev"]
#    else:
#	newslicestr = defs["pd_dev"] + ":" + defs["tf_slice"]
#    
#    edit_sub(defs,oldslicestr,newslicestr);
##end os_fsmnt_tf_slice_edit_std
#
#def os_fsmnt_pd_type_edit_std(defs):
#    print 'os_fsmnt_pd_type_edit_std()';
#
#    oldstr = defs["LAST_PD_TYPE"];
#    newstr = defs["pd_type"];
#    
#    edit_sub(defs,oldstr,newstr);
##end os_fsmnt_pd_type_edit_std
#

#def os_fsmnt_tf_nmps_readin_std(defs):
#    # scan the mounttable file
#    # find all mount points
#    # create nodes for them in gui
#    print 'os_fsmnt_tf_nmps_readin_std'
#
#    mnt_name = "/dir1";
#    npw2, uid = getNPW2Connection(defs);
#    npw2.ReadIn_AddChildComponent(uid,"integrity_os_modules_file_system_mntpoint",mnt_name,False);
##end os_fsmnt_tf_nmps_readin_std


def fs_find_mounttable(defs,all_c_files,tell_mprojmgr=True):
...
#	    newi1 = re.search("\"(?P<device>\w+):(?P<slice>\w+)\",",i);
	    if (newi1 != None):
		phase = 3;
...
def fs_tf_rdsize_sanity_check(defs):
    ...
    if (defs["bt_rd"] != "off" and not re.search("^(?P<sizestr>[0-9]+[KMGkmg]?)$",sizestr)):
	npw2, uid = getNPW2Connection(defs);
        failstr = "RAMDisk must be of the form <number><unit>, e.g. 2M or 128K";
        npw2.ReadInString(uid,failstr);
        return;
