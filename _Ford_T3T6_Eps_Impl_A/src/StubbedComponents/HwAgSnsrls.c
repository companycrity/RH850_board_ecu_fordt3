 ...
 *  -------------------------------------------------------------------------------------------------------------------
 *          File:  HwAgSnsrls.c
 *        Config:  C:/_Synergy/T3T6/T3T6_0.2.13/SF042A_HwAgSnsrls_Impl/tools/Component.dpa
 *     SW-C Type:  HwAgSnsrls
 *  Generated at:  Mon Mar 26 11:29:12 2018
 *
...
* 03/24/17     6       ML		Updated Rev as per design 2.0.0												  EA4#9330
* 08/15/17     7       ML       Added new Inputs, Calibrations, and Logic involving them                      EA4#14091
***********************************************************************************************************************/
...
static FUNC(void,  HwAgSnsrls_CODE) EcuIdentifier(uint8 EcuId_Cnt_T_u08, 
 ...
 *   NTCNR_0X1FE (510U)
 *   NTCNR_0X1FF (511U)
 * SelnCaseSt1: Enumeration of integer in interval [0...255] with enumerators
 *   SELNCASEST_FCENTR (0U)
 *   SELNCASEST_VEHDYN (1U)
 *   SELNCASEST_WHLSPD (2U)
 *   SELNCASEST_STORDLSTPOSN (3U)
 *   SELNCASEST_CORRDPINIONAG (4U)
 ...
 *   SelnCaseSt1 *Rte_Pim_SelnCaseSt(void)
 ...
 *   ---------------------------------------------
 *   float32 Rte_Prm_HwAgSnsrlsCorrdPinionAgConfThd_Val(void)
 ...
 *   uint8 Rte_Prm_HwAgSnsrlsMotAgIdptSigThd_Val(void)
 *   boolean Rte_Prm_HwAgSnsrlsFeatEna_Logl(void)
 *
 *********************************************************************************************************************/
 ...
 *   Std_ReturnType Rte_Call_GetRefTmr100MicroSec32bit_Oper(uint32 *RefTmr_Arg)
 *     Synchronous Server Invocation. Timeout: None
 *   Std_ReturnType Rte_Call_GetSigImcData_Oper(uint16 SigId_Arg, float32 *Data_Arg, ImcArbnRxSts1 *Sts_Arg)
 *     Synchronous Server Invocation. Timeout: None
 ...
 *********************************************************************************************************************/
    ...
    (void)Rte_Read_MotAgCumvAlgndVld_Logl(&MotAgCumvAlgndVld_Cnt_T_logl);        
    (void)Rte_Read_MotAgCumvAlgndCrf_Val(&MotAgCumvAlgndCrf_MotDeg_T_s18p13);    
   ...
   EcuIdentifier(EcuId_Cnt_T_u08,
	...
	if(PinionAgConf_Uls_T_f32 >= Rte_Prm_HwAgSnsrlsCorrdPinionAgConfThd_Val())
	{
    ...
    if(MotAgMeclIdptSig_Cnt_T_u08 <= Rte_Prm_HwAgSnsrlsMotAgIdptSigThd_Val())
    {
        LrndHwConf_Uls_T_f32 = 0.0F;
    }        
        
	/*** Range Limit and Write Outputs to RTE ***/
	(void)Rte_Write_HwAgSnsrls_Val(Lim_f32(*Rte_Pim_PrevSmotngHwAg(), HWAGLOLIM_HWDEG_F32, HWAGHILIM_HWDEG_F32)); 
    (void)Rte_Write_HwAgSnsrlsConf_Val(Lim_f32(LrndHwConf_Uls_T_f32, HWCONFLOLIM_ULS_F32, HWCONFHILIM_ULS_F32));  
...
* Inputs      :   WhlFrqVld_Cnt_T_logl - RTE Input
*                 WhlLeFrq_Hz_T_f32 - RTE Input
*                 WhlRiFrq_Hz_T_f32 - RTE Input
*                 VehSpd_Kph_T_f32 - RTE Input
*                 RelHwAg_HwDeg_T_f32 - Calculated in Per1
												   ...
												   P2VAR(float32, HwAgSnsrls_CODE, AUTOMATIC) WhlSpdHwConf_Uls_T_f32)
{
    ...
    VAR(float32, AUTOMATIC) AbsltWhlLeFrq_Hz_T_f32;
    VAR(float32, AUTOMATIC) AbsltWhlRiFrq_Hz_T_f32;
	
	/*** Start of WhlSpdAutoCentr ***/
	/*** WhlSpd ***/
	AbsltWhlLeFrq_Hz_T_f32 = Abslt_f32_f32(WhlLeFrq_Hz_T_f32);
    AbsltWhlRiFrq_Hz_T_f32 = Abslt_f32_f32(WhlRiFrq_Hz_T_f32);
    
	if(TRUE == WhlFrqVld_Cnt_T_logl)
    {
        if((AbsltWhlLeFrq_Hz_T_f32 > Rte_Prm_HwAgSnsrlsWhlFrqThd_Val()) &&
		   (AbsltWhlRiFrq_Hz_T_f32 > Rte_Prm_HwAgSnsrlsWhlFrqThd_Val()))
        {
...
* Inputs      :   MotTqCmdCrf_MotNwtMtr_T_f32 - RTE Input
*                 HwTq_HwNwtMtr_T_f32 - RTE Input
*                 VehYawRate_VehDegPerSec_T_f32 - RTE Input  
*                 MotVelCrf_MotRadPerSec_T_f32 - RTE Input
*                 VehSpdVld_Cnt_T_logl - RTE Input
*                 VehSpd_Kph_T_f32 - RTE Input
*                 RelHwAg_HwDeg_T_f32 - Calculated in Per1
*                 VehYawRateVld_Cnt_T_logl - RTE Input
...
*                 MotVelCrf_MotRadPerSec_T_f32 - RTE Input
*                 VehSpdVld_Cnt_T_logl - RTE Input
*                 VehSpd_Kph_T_f32 - Calculated in Per1
*                 RelHwAg_HwDeg_T_f32 - RTE Input
*                 VehYawRateVld_Cnt_T_logl - RTE Input
                                                                 ...
                                                                 boolean VehYawRateVld_Cnt_T_logl)
{
    ...
    VAR(float32, AUTOMATIC) AbsltFildPinionTq_Uls_T_f32;
    VAR(float32, AUTOMATIC) AbsltVehSpd_Kph_T_f32;
    VAR(float32, AUTOMATIC) AbsltMotVelCrf_MotRadPerSec_T_f32;
    VAR(float32, AUTOMATIC) AbsltYawRate_Uls_T_f32;
    ...
    AbsltFildPinionTq_Uls_T_f32 = Abslt_f32_f32(FildPinionTq_Uls_T_f32);
    AbsltVehSpd_Kph_T_f32 = Abslt_f32_f32(VehSpd_Kph_T_f32);
    AbsltMotVelCrf_MotRadPerSec_T_f32 = Abslt_f32_f32(MotVelCrf_MotRadPerSec_T_f32);
    AbsltYawRate_Uls_T_f32 = Abslt_f32_f32(YawRate_Uls_T_f32);
    

	if( (AbsltFildPinionTq_Uls_T_f32 <= Rte_Prm_HwAgSnsrlsVehDynPinionTqThd_Val()) &&      
	    (AbsltVehSpd_Kph_T_f32 >= Rte_Prm_HwAgSnsrlsVehDynVehSpdThd_Val()) &&              
	    (AbsltMotVelCrf_MotRadPerSec_T_f32 <= Rte_Prm_HwAgSnsrlsVehDynMotVelThd_Val()) &&  
	    (AbsltYawRate_Uls_T_f32 <= Rte_Prm_HwAgSnsrlsVehDynYawRateThd_Val()) &&            
...
* Inputs      :   FCentrHwConf_Uls_T_f32 - Calculated in Per1
*                 VehDynHwConf_Uls_T_f32 - Calculated in 'VehDynAutoCentr'
*                 WhlSpdHwConf_Uls_T_f32 - Calculated in 'WhlSpdAutocentr' 
*                 RelHwAg_HwDeg_T_f32 - Calculated in Per1
...
static FUNC(void,  HwAgSnsrls_CODE) ArbtrtnSmthng(float32 FCentrHwConf_Uls_T_f32, 
												  ...
												  P2VAR(float32, HwAgSnsrls_CODE, AUTOMATIC) LrndHwConf_Uls_T_f32)
{
	VAR(SelnCaseSt1, AUTOMATIC) SelnCaseSt_Cnt_T_enum;
	VAR(SelnCaseSt1, AUTOMATIC) SelnCaseSt1_Cnt_T_enum;
	VAR(SelnCaseSt1, AUTOMATIC) SelnCaseSt2_Cnt_T_enum;
	VAR(SelnCaseSt1, AUTOMATIC) SelnCaseSt3_Cnt_T_enum;
	...
	if(*LrndHwConf_Uls_T_f32 <= *Rte_Pim_PrevMaxHwConf())
	{
		...
		*LrndHwConf_Uls_T_f32 = *Rte_Pim_PrevMaxHwConf();
		SelnCaseSt_Cnt_T_enum = *Rte_Pim_SelnCaseSt();
	}
	else
	{
		/*** Start of HwAgOffsSeln ***/
		switch(SelnCaseSt_Cnt_T_enum)
		{
			case SELNCASEST_FCENTR:
			{
				LrndHwAg_HwDeg_T_f32 = *Rte_Pim_FCentrHwAgOffs();
			}
			break;
			
			case SELNCASEST_VEHDYN:
			{
				LrndHwAg_HwDeg_T_f32 = *Rte_Pim_VehDynHwAgOffs();
			}
			break;
			
			case SELNCASEST_WHLSPD:
			{
				LrndHwAg_HwDeg_T_f32 = *Rte_Pim_WhlSpdHwAgOffs();
			}
			break;
			
			case SELNCASEST_STORDLSTPOSN:
			{
		        LrndHwAg_HwDeg_T_f32 = -(StordHwAg_Cnt_T_f32);
			}
			break;
			
			default: /*SELNCASEST_CORRDPINIONAG */
			{
	...
	*Rte_Pim_PrevHwAgOffs() = LrndHwAg_HwDeg_T_f32;
	LrndHwAg_HwDeg_T_f32 = RelHwAg_HwDeg_T_f32 - LrndHwAg_HwDeg_T_f32;
	...
	*Rte_Pim_PrevMaxHwConf() = *LrndHwConf_Uls_T_f32;
    ...
    if( (Abslt_f32_f32(LrndHwAgDif_HwDeg_T_f32) > Rte_Prm_HwAgSnsrlsSmotngStepThd_Val()) &&
	    ((*Rte_Pim_SelnCaseSt() != SelnCaseSt_Cnt_T_enum) || (TRUE == *Rte_Pim_PrevSmotngSt())) )
	{
	...
	*Rte_Pim_SelnCaseSt() 	  = SelnCaseSt_Cnt_T_enum;
												  ...
												  float32 RelHwAg_HwDeg_T_f32)
{
	...
	VAR(ImcArbnRxSts1, AUTOMATIC) ImcPinionAgConfSts_Cnt_T_enum;
	VAR(boolean, AUTOMATIC) ImcPinionAgConfVld_Cnt_T_logl;
	VAR(float32, AUTOMATIC) ImcPinionAg_HwDeg_T_f32;
	VAR(ImcArbnRxSts1, AUTOMATIC) ImcPinionAgSts_Cnt_T_enum;
	VAR(boolean, AUTOMATIC) ImcPinionAgVld_Cnt_T_logl;
	VAR(Std_ReturnType, AUTOMATIC) ImcPinionAgConfRtn_Cnt_T_enum;
	VAR(Std_ReturnType, AUTOMATIC) ImcPinionAgRtn_Cnt_T_enum; 

	if((EcuId_Cnt_T_u08 == 2U)&&(Rte_Prm_HwAgSnsrlsFeatEna_Logl() == TRUE))
	{
		ImcPinionAgConfRtn_Cnt_T_enum = Rte_Call_GetSigImcData_f32_Oper(IMCARBN_HWAGSNSRLSPINIONAGCONF_CNT_U16,&ImcPinionAgConf_Uls_T_f32,&ImcPinionAgConfSts_Cnt_T_enum);
		if((ImcPinionAgConfSts_Cnt_T_enum == IMCARBNRXSTS_VLD) && (ImcPinionAgConfRtn_Cnt_T_enum != E_NOT_OK))
		{
			ImcPinionAgConfVld_Cnt_T_logl = TRUE;
		}
		else
		{
			ImcPinionAgConfVld_Cnt_T_logl = FALSE;
		}
		
		ImcPinionAgRtn_Cnt_T_enum = Rte_Call_GetSigImcData_f32_Oper(IMCARBN_HWAGSNSRLSPINIONAG_CNT_U16,&ImcPinionAg_HwDeg_T_f32,&ImcPinionAgSts_Cnt_T_enum);
		if((ImcPinionAgSts_Cnt_T_enum == IMCARBNRXSTS_VLD) && (ImcPinionAgRtn_Cnt_T_enum != E_NOT_OK))
		{
			ImcPinionAgVld_Cnt_T_logl = TRUE;
		}
		else
		{
			ImcPinionAgVld_Cnt_T_logl = FALSE;
		}
		
		if((ImcPinionAgVld_Cnt_T_logl == TRUE)&&(ImcPinionAgConfVld_Cnt_T_logl == TRUE))
		{
			if(ImcPinionAgConf_Uls_T_f32 >= Rte_Prm_HwAgSnsrlsCorrdPinionAgConfThd_Val())
			{
				*Rte_Pim_CorrdPinionAgHwAgOffs() = RelHwAg_HwDeg_T_f32 - ImcPinionAg_HwDeg_T_f32;
				*Rte_Pim_CorrdPinionAgHwConf() = Rte_Prm_HwAgSnsrlsCorrdPinionAgHwConf_Val();
			}
		}
	}
