...
typedef enum
{
  ...
  FEE_30_SMALLSECTOR_DE_STATE_WRITE_ERASE_PATTERN,
  FEE_30_SMALLSECTOR_DE_STATE_ERASE_SECTOR
...
FEE_30_SMALLSECTOR_LOCAL VAR(MemIf_JobResultType, FEE_30_SMALLSECTOR_APPL_DATA) Fee_30_SmallSector_De_InstanceFinderResult;
...
/**********************************************************************************************************************
 Fee_30_SmallSector_De_ProcessStateWriteErasePattern()
 **********************************************************************************************************************/
/*!
 * \brief      Processes WriteErasePattern state of DatasetEraser state machine
 * \details    Initiates erase of sector
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_De_ProcessStateWriteErasePattern(void);
...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_De_EraseLastSector(void)
{
  ...
  {
    Fee_30_SmallSector_De_ComponentParameter.StateMachine = FEE_30_SMALLSECTOR_DE_STATE_ERASE_SECTOR;
  }
  else
  {
...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_De_ProcessStateIdle(void)
{
  /* #10 ErasePattern needs to be written to reserved page if InstanceFinder sub-component finished its job successfully */
  if (Fee_30_SmallSector_De_InstanceFinderResult == MEMIF_JOB_OK)
  {
    /* #20 Write ErasePattern to first page of dataset */
    if (Fee_30_SmallSector_Dh_WriteErasePattern() == E_OK)
    {
      Fee_30_SmallSector_De_ComponentParameter.StateMachine = FEE_30_SMALLSECTOR_DE_STATE_WRITE_ERASE_PATTERN;
    }
    else
    {
      Fee_30_SmallSector_De_FinishJob(MEMIF_JOB_FAILED);
  ...
  /* #30 InstanceFinder job returned with MEMIF_JOB_INCONSISTENT, which indicates that ErasePattern is not blank. */
  /* #31 Therefore it's not necessary to write the ErasePattern. Dataset's Flash sectors can be erased immediately */
  else
  {
    /* #40 Start erasing the Flash sectors */
    Fee_30_SmallSector_De_EraseLastSector();
  }
}
...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_De_ProcessStateWriteErasePattern(void)
{
  /* #10 Flash finished writing the erase pattern */
...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_De_ProcessStateEraseSector(void)
{
  ...
  if (Fee_30_SmallSector_Fls_GetJobResult() == MEMIF_JOB_OK)
  {
    ...
    /* #50 Finish sub-component job, because all sectors were successfully erased */
    else
    {
      Fee_30_SmallSector_De_FinishJob(MEMIF_JOB_OK);
    }
  }
  else
  {
...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_De_ProcessStateMachine(void)
{
  switch (Fee_30_SmallSector_De_ComponentParameter.StateMachine)
  {
    ...
    case FEE_30_SMALLSECTOR_DE_STATE_IDLE:
      ...
      /*** ---------------- WRITE ERASE PATTERN ----------------------------------------------------------- ***/
    case FEE_30_SMALLSECTOR_DE_STATE_WRITE_ERASE_PATTERN:
      Fee_30_SmallSector_De_ProcessStateWriteErasePattern();
      break;

      /*** ---------------- ERASE SECTOR ------------------------------------------------------------------ ***/
    case FEE_30_SMALLSECTOR_DE_STATE_ERASE_SECTOR:
      Fee_30_SmallSector_De_ProcessStateEraseSector();
      break;
...
FUNC(void, FEE_30_SMALLSECTOR_PUBLIC_CODE) Fee_30_SmallSector_De_Init(void)
{
  ...
  Fee_30_SmallSector_De_InstanceFinderResult = MEMIF_JOB_OK;
 ...
 *
 */
FUNC(Std_ReturnType, FEE_30_SMALLSECTOR_PUBLIC_CODE) Fee_30_SmallSector_De_StartJob(MemIf_JobResultType InstanceFinderResult)
{
