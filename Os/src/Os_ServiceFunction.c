 ...
 *                  returned.
 *
 *  \param[in]      ServiceFunction       Service to query.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in]      StackPool           StackPool which shall be accessed.
 *  \param[in]      CurrentApplication  Application which wants to access.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in]      Service   Service to be queried.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in]      Service   Service to be converted.
 *
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Given service is a trusted function
 **********************************************************************************************************************/
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Given service is a non trusted function.
 **********************************************************************************************************************/
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Given service call is a non trusted function call.
 **********************************************************************************************************************/
 ...
 *  \details        --no details--
 *
 *  \param[in]      TrustedFunction   Trusted Function to query.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  ServiceCall     Where to store previous info.
 *  \param[in]      Service         Service where to load the new settings from.
 *  \param[in,out]  CurrentThread   Thread which shall be modified.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  StackPool           Stack pool to initialize.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  StackPool           Stack pool to get the stack from.
 ...
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Concurrent access to given object is prevented by caller.
 *  \pre            CurrentApplication is valid.
 **********************************************************************************************************************/
 ...
 *
 *  \param[in,out]  StackPool     Stack pool to get the stack from.
 *  \param[in]      Stack         Stack which shall be released.
 *  \param[in]      Application   Application which wants to release the stack.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Stack has to belong to the StackPool.
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  TrustedFunction   Trusted Function which shall be executed.
 *  \param[in]      FunctionIndex     FunctionIndex parameter to be passed.
 *  \param[in]      FunctionParams    FunctionParams parameter to be passed.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  NonTrustedFunction  Non-Trusted Function which shall be executed.
 *  \param[in]      FunctionIndex       FunctionIndex parameter to be passed.
 *  \param[in]      FunctionParams      FunctionParams parameter to be passed.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Service The service to initialize.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  NonTrustedFunction   NonTrustedFunction to initialize.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
...
)
{
  ...
  if(OS_UNLIKELY(Os_ServiceCheckIsAccessible(&(TrustedFunction->Base)) == OS_CHECK_FAILED))                             /* SBSW_OS_SF_SERVICECHECKISACCESSIBLE_001 */
  {
  ...
  else
  {
    ...
    /* #70 Trusted function specific call behavior. */
    {
      ...
      if(Os_ServiceTfNeedsModeSwitch(TrustedFunction) != 0)                                                             /* SBSW_OS_FC_PRECONDITION */
      {
        ...
        /*! \note ESCAN00088830: Missing memory protection switches in configuration: memory protection for trusted
         *                       applications. */
...
)
{
  ...
  /* #30 Check whether the service is accessible. */
  if(OS_UNLIKELY(Os_ServiceCheckIsAccessible(&(NonTrustedFunction->Base)) == OS_CHECK_FAILED))                          /* SBSW_OS_SF_SERVICECHECKISACCESSIBLE_001 */
  {
    status = OS_STATUS_NOTACCESSIBLE_1;
  }
...
)
{
  ...
  if(OS_UNLIKELY(Os_ServiceCheckTfId(FunctionIndex) == OS_CHECK_FAILED))
  {
    status = OS_STATUS_SERVICEID;
  }
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_CALLTRUSTEDFUNCTION) == OS_CHECK_FAILED))  /* SBSW_OS_SF_THREADCHECKCALLCONTEXT_001 */
  {
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(ownerCore, localCore) == OS_CHECK_FAILED))                                   /* SBSW_OS_SF_CORECHECKISLOCALCORE_001 */
    {
...
)
{
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_CALLNONTRUSTEDFUNCTION)                    /* SBSW_OS_SF_THREADCHECKCALLCONTEXT_001 */
      == OS_CHECK_FAILED))
  {
    status = OS_STATUS_CALLEVEL;
  }
