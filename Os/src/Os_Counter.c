 ...
 *
 *  \param[in]    Counter       The counter which holds the range limitation info.
 ...
 *
 *  \param[in,out]  Counter         The counter to modify.
 ...
 *  \param[in]      CallerCore  The core, which is performing this call.
 *  \param[out]     Value       Value variable .
 *
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Counter         The counter to query.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
...
)
{
  ...
  while(OS_LIKELY(job != NULL_PTR))
  {
    ...
    if(OS_LIKELY(Os_CounterIsFutureValue(Counter, job->Dyn->ExpirationTimestamp, now) == 0))                            /* SBSW_OS_FC_PRECONDITION */
    {
      ...
      /*! \note Call should be performed by job module. */
      /* #50 Work the job off. */
...
)
{
  ...
  if(topJob == Job)
  {
    ...
    /*! \note Code may be combined with WorkJobs in a separate function. */
    if(topJob == NULL_PTR)
    {
      Os_TickType now = Os_CounterGetPhysicalValue(Counter);                                                            /* SBSW_OS_FC_PRECONDITION */
...
)
{
  ...
  else
  {
    ...
    else if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(me->Core, localCore) == OS_CHECK_FAILED))                               /* SBSW_OS_CNT_CORECHECKISLOCALCORE_001 */
    {
