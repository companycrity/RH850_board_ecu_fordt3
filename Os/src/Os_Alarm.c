 ...
 *
 *  \param[in,out]  Alarm   Alarm which has arrived.
 *
 *  \context        OS_INTERNAL
 *
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Alarm     Alarm to set.
 ...
 *  \details        --no details--
 *
 *  \param[in]      Alarm     Alarm to read from.
 *  \param[out]     Tick      The current alarm value.
 *
 *  \retval         OS_STATUS_OK              No error.
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Alarm   Alarm to cancel.
 *
 *  \retval         OS_STATUS_OK                No error.
 *  \retval         OS_STATUS_NOFUNC_1          Alarm is not in use.
 *  \retval         OS_STATUS_NOTACCESSIBLE_1   (Service Protection:) Given alarm's owner application is not accessible.
...
)
{
  ...
  if((autostartConfig->ApplicationModes & AppMode) != 0)
  {
    ...
    /*! \note Does it make sense to use absolute alarms during startup?? The system should
     *        always behave like SetRelAlarm() I think. */
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(targetCore == Os_ThreadGetCore(currentThread))                                                                 /* SBSW_OS_AL_THREADGETCORE_001 */
      {
