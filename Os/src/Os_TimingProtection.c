 ...
 *
 *  \note           Passing Owner and Dyn as two values, enables the caller to pass to values, which do not belong
 *                  together. Check whether passing only the Owner pointer is sufficient.
 **********************************************************************************************************************/
...
)
{
  ...
  if(Os_TpIsEnabled() != 0)
  {
    /*! \note Os_TimerHrtGetCounter is called multiple times. Do it once. */
...
ISR(Os_TpIsr)
{
  /*! \note   Problem with using the regular Isr-Wrapper for timing protection:
   *          - Settings are restored from configuration instead of hardware (i.e. MPU configuration of ISR1 gets lost).
   */
  /*! \note Os_TimerHrtGetCounter is called multiple times. Do it once. */
