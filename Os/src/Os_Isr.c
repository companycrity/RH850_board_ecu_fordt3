...
)
{
  ...
  if(OS_LIKELY(Os_TpCheckInterArrival(timingProtection, Os_ThreadGetTpConfig(&Isr->Thread)) != OS_CHECK_FAILED))        /* SBSW_OS_ISR_TPCHECKINTERARRIVAL_001 */ /* SBSW_OS_FC_PRECONDITION */
  {
    ...
    Os_ThreadSuspendAndStart(interruptedThread, &Isr->Thread);                                                          /* SBSW_OS_ISR_THREADSUSPENDANDSTART_001 */
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_IsrEpilogue, (void))
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_ThreadCheckSpinlocksReleased(isrThread) == OS_CHECK_FAILED))                                      /* SBSW_OS_ISR_THREADCHECKSPINLOCKSRELEASED_001 */
    {
  ...
  else
  {
    ...
    Os_ThreadCleanupAndResume(interruptedThread);                                                                       /* SBSW_OS_ISR_THREADCLEANUPANDRESUME_001 */
  }
