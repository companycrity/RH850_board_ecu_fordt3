 ...
 *                  ActivationsAllowed.
 *
 *  \param[in,out]  Task    The task which shall be reset.
 *
 *  \context        ANY
 *
...
FUNC(Os_StatusType, OS_CODE) Os_Api_TerminateTask(void)                                                                 /* PRQA S 1503 */ /* MD_MSR_14.1 */
{
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckSpinlocksReleased(currentThread) == OS_CHECK_FAILED))                               /* SBSW_OS_TASK_THREADCHECKSPINLOCKSRELEASED_001 */
  {
