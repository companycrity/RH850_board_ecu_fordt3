 ...
 *  \param[in]      TargetApp         Application to terminate.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  App    The application to terminate.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      FALSE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Thread The thread to terminate.
 *  \param[in]      App    The application to compare with.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      FALSE
 ...
 *
 *  \param[in,out]  App             The application to terminate.
 *  \param[in]      RestartOption   Termination kind: Terminate or restart.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      FALSE
 ...
 *  \param[in]      CallerCurrentApplication  The original caller of the API.
 *
...
)
{
  ...
  else
  {
    ...
    if(OS_LIKELY(Os_AppIsMemberOfCore(targetApp, Os_ThreadGetCore(currentThread)) != 0))                                /* SBSW_OS_APP_APPISMEMBEROFCORE_001 */ /* SBSW_OS_APP_THREADGETCORE_001 */
    {
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY((appState == APPLICATION_RESTARTING) && (CallerCurrentApplication != App)))
    {
    ...
    else if(OS_UNLIKELY(appState == APPLICATION_TERMINATED))
    {
...
\CM CM_OS_APP_ALARMREF_M
      Verify that:
        ...
        3. each element in AlarmRefs of each application is no NULL_PTR.
...
\CM CM_OS_APP_COUNTERREF_M
      Verify that:
        ...
        3. each element in CounterRefs of each application is no NULL_PTR.
...
\CM CM_OS_APP_ISRCAT1REF_M
      Verify that:
        ...
        3. each element in IsrCat1Refs of each application is no NULL_PTR.
...
\CM CM_OS_APP_ISRCAT2REF_M
      Verify that:
        ...
        3. each element in IsrCat2Refs of each application is no NULL_PTR.
...
\CM CM_OS_APP_SCHTREF_M
      Verify that:
        ...
        3. each element in SchTRefs of each application is no NULL_PTR.
...
\CM CM_OS_APP_TASKREF_M
      Verify that:
        ...
        3. each element in TaskRefs of each application is no NULL_PTR.
...
\CM CM_OS_APP_SERIVCEFUNCTION_M
      Verify that:
        ...
        3. each element in ServiceFunctions of each application is no NULL_PTR.
*/
