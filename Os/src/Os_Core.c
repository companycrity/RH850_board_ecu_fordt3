...
#if (  (OS_SW_MAJOR_VERSION != (1u)) \
    || (OS_SW_MINOR_VERSION != (8u)) \
 ...
 *
 *  \param[in,out]  Core    Core to query.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *
 *  \param[in,out]  Core    Core to initialize.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *                  applications.
 *
 *  \param[in,out]  Core     The core to initialize.
 *
 *  \context        OS_INTERNAL
 *
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Given Id is in a valid range.
 **********************************************************************************************************************/
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Core              The core to query.
 ...
 *
 *  \param[in]    Id  The Id of this core, which should emulates slave.
 *
 *  \context      OS_INTERNAL
 *
 *  \reentrant    FALSE
...
OS_LOCAL FUNC(void, OS_CODE) Os_CorePreStartInit(void)
{
  ...
  /* #10 Initialize stacks. */
  {
    Os_ObjIdxType stackIdx;

    for(stackIdx = 0; stackIdx < core->StackCount; ++stackIdx)
    {
      Os_StackInit(core->StackRefs[stackIdx]);                                                                          /* SBSW_OS_CORE_STACKINIT_001 */
    }
  }

  /* #40 Initialize core's memory protection. */
  Os_MpCoreInit(core->MemoryProtection);                                                                                /* SBSW_OS_CORE_MPCOREINIT_001 */

  /* #50 Initialize system hooks. */
  Os_CoreHooksInit(core);                                                                                               /* SBSW_OS_CORE_COREHOOKSINIT_001 */

  /* #60 Initialize timing protection with the Idle Task. */
...
)
{
  ...
  /* #10 Initialize system's memory protection. */
  Os_MpSystemInit(systemCfg->MemoryProtection);                                                                         /* SBSW_OS_CORE_MPSYSTEMINIT_001 */

  /* #20 Initialize the user spinlocks. */
  ...
  /* #30 For each core: */
  {
    ...
    for(coreId = (CoreIdType)0; coreId < OS_COREID_COUNT; coreId++)
    {
      ...
      /* #50 If the core is an AUTOSAR core: */
      if(Os_CoreIsAsrCore(coreIt) != 0)                                                                                 /* SBSW_OS_CORE_COREISASRCORE_001 */
      {
        ...
        /* #60 Set initial AUTOSAR core status. */
        ...
        /* #70 Initialize core barrier. */
        Os_BarrierInit(asrCore->Barrier, OS_BARRIERATTENDEESTATE_DETACHED);                                             /* SBSW_OS_CORE_BARRIERINIT_001 */

        Os_AppSetNotAccesible(asrCore->KernelApp);                                                                      /* SBSW_OS_CORE_APPSETNOTACCESIBLE_001 */

        /* #80 Initialize the OS state of each application AUTOSAR core as 'not started'. */
        for(appIterator = (Os_ObjIdxType)0; appIterator < asrCore->AppCount; appIterator++)
        {
          Os_AppSetNotAccesible(asrCore->AppRefs[appIterator]);                                                         /* SBSW_OS_CORE_APPSETNOTACCESIBLE_002 */
        }
      }
    }
  }

  /* #90 Initialize the IOCs which are not assigned to a specific core. */
  {
    Os_ObjIdxType it;

    for(it = 0; it < systemCfg->IocCount; it++)
    {
      Os_IocInit(systemCfg->IocRefs[it]);                                                                               /* SBSW_OS_CORE_IOCINIT_001 */
    }
  }
}                                                                                                                       /* PRQA S 6050 */ /* MD_Os_STCAL */
...
FUNC(OS_LOCAL_INLINE void, OS_CODE) Os_CoreInit(void)
{
  ...
  {
    Os_IocInit(core->IocRefs[index]);                                                                                   /* SBSW_OS_CORE_IOCINIT_002 */
  }
  ...
  /* #60 Synchronize with started AUTOSAR cores. */
  Os_BarrierSynchronize(core->Barrier);                                                                                 /* SBSW_OS_CORE_BARRIERSYNCHRONIZE_001 */

  /* #70 Call system startup hook. */
  (void)Os_HookCallCallback(core->StartupHookRef);                                                                      /* SBSW_OS_CORE_HOOKCALLCALLBACK_001 */

  /* Kernel App's hooks are may not be called. */

  /* #80 Call application specific startup hooks. */
  for(index = 0; index < core->AppCount; index++)
  {
...
))
{
  ...
  {
    /* #30 Enter endless loop with disabled interrupts. */
    Os_ErrKernelPanic();
  }
  ...
  {
    /* Kernel App's Os_AppKillNoSuicide may not be called. */
    ...
    /* Kernel App's Os_AppKillNoSuicide are may not be called. */
  ...
  /* #110 Enter endless loop with disabled interrupts. */
  Os_ErrKernelPanic();
}                                                                                                                       /* PRQA S 6050 */ /* MD_Os_STCAL */
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_CoreShutdownAllLocal,
(
  StatusType Error
))
{
...
))
{
  ...
  for(coreIdx = (CoreIdType)0; coreIdx < OS_COREID_COUNT; ++coreIdx)
  {
    if(coreIdx != currentCoreId)
    {
      ...
      if(core->Status->ActivationState == OS_CORESTATE_ACTIVATED_ASR)
      {
...
{
  Os_CoreAsrGetAsrStatus(Core)->ApplicationMode |= ApplicationMode;                                                     /* SBSW_OS_FC_PRECONDITION */ /* SBSW_OS_CORE_COREGETSTATUS_002 */
}
...
)
{
  ...
  for(interruptSourceIdx = (Os_ObjIdxType)0; interruptSourceIdx < Core->InterruptSourceCount; interruptSourceIdx++)
  {
...
OS_LOCAL FUNC(void, OS_CODE) Os_CoreEmulateMaster(void)
{
  ...
  /* #10 For all non auto-started slave cores. */
  for(coreId = (CoreIdType)0; coreId < OS_COREID_COUNT; coreId++)
  {
  ...
  for(coreId = (CoreIdType)0; coreId < OS_COREID_COUNT; coreId++)
  {
    if(coreId != OS_CORE_ID_MASTER)
    {
      ...
      if(core->IsAutostart == TRUE)
      {
        ...
        while(OsCfg_CoreBootBarrierRefs[coreId]->WaitingSign != OS_CORE_BOOTBARRIER_INITHARDWARE_COMPLETED)
        {
          Os_Hal_CoreNop();
        }
      }
 ...
 *
 */
OS_LOCAL FUNC(void, OS_CODE) Os_CoreEmulateSlaveWaitForHardwareInit(CoreIdType Id)
{
  P2CONST(Os_CoreConfigType, AUTOMATIC, OS_CONST) core;
 ...
 *
 */
OS_LOCAL FUNC(void, OS_CODE) Os_CoreEmulateSlaveWaitForStartCore(CoreIdType Id)
{
  P2CONST(Os_CoreConfigType, AUTOMATIC, OS_CONST) core;
  ...
  if(core->IsAutostart == TRUE)
  {
    ...
    while(OsCfg_CoreBootBarrierRefs[Id]->WaitingSign != OS_CORE_BOOTBARRIER_STARTCORE)
    {
      Os_Hal_CoreNop();
    }
  }
...
FUNC(void, OS_CODE) Os_Api_Init(void)
{
  ...
  /* #10 If we are the master core. */
  if(core->Id == OS_CORE_ID_MASTER)
  {
    /* #20 Perform the boot synchronization. */
    Os_CoreEmulateMaster();

    /* #30 Perform hardware specific initialization on the master core. */
    if(core->HasPrivilegedHardwareAccess == TRUE)
    {
      Os_Hal_CoreInitMaster(systemCfg->HwConfig);                                                                       /* SBSW_OS_CORE_HAL_COREINITMASTER_001 */
    }

    /* #40 Initialize interrupt handling. */
    Os_CoreInterruptHandlingInit(core);                                                                                 /* SBSW_OS_CORE_COREINTERRUPTHANDLINGINIT_001 */

    /* #50 Initialize the system data. */
    Os_SystemInit();
  }
  /* #60 Otherwise: */
  else
  {
    ...
    /* #70 Start the master core, if it is not an auto-start core and the current core is allowed to do that. */
    ...
    /* #80 Perform the boot synchronization on this slave core. */
    Os_CoreEmulateSlaveWaitForHardwareInit(core->Id);

    /* #90 Perform hardware specific initialization on the slave core. */
    if(core->HasPrivilegedHardwareAccess == TRUE)
    {
      Os_Hal_CoreInitMaster(systemCfg->HwConfig);                                                                       /* SBSW_OS_CORE_HAL_COREINITMASTER_001 */
    }

    /* #100 Initialize interrupt handling. */
    Os_CoreInterruptHandlingInit(core);                                                                                 /* SBSW_OS_CORE_COREINTERRUPTHANDLINGINIT_001 */
  ...
  /* #130 If we are an AUTOSAR core: */
  if(core->IsAsrCore == TRUE)
  {
    P2CONST(Os_CoreAsrConfigType, AUTOMATIC, OS_CONST) asrCore;

    asrCore = Os_Core2AsrCore(core);                                                                                    /* SBSW_OS_CORE_CORE2ASRCORE_001 */

    /* #140 If we are the master core. */
    if(core->Id == OS_CORE_ID_MASTER)
    {
      /* #150 Attach to the barrier. */
    ...
    /* #160 Perform reduced initialization that allows usage of interrupt API. */
    Os_CoreReducedInit(asrCore);                                                                                        /* SBSW_OS_CORE_COREREDUCEDINIT_001 */
  }
  /* #170 Otherwise if this is the master core, update its state. */
  else if(core->Id == OS_CORE_ID_MASTER)
  {
...
)
{
  ...
  Os_BarrierSynchronize(coreAsr->Barrier);                                                                              /* SBSW_OS_CORE_BARRIERSYNCHRONIZE_002 */
...
)
{
  ...
  else if(OS_UNLIKELY(Os_CoreCheckIdleMode(IdleMode) == OS_CHECK_FAILED))
  {
    status = OS_STATUS_ID_2;
  }
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_CONTROLIDLE) == OS_CHECK_FAILED))          /* SBSW_OS_CORE_THREADCHECKCALLCONTEXT_001 */
  {
    status = OS_STATUS_CALLEVEL;
  }
  else if(OS_UNLIKELY(Os_ThreadCheckAreInterruptsEnabled(currentThread) == OS_CHECK_FAILED))                            /* SBSW_OS_CORE_THREADCHECKAREINTERRUPTSENABLED_001 */
  {
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsAsrCore(targetCore) == OS_CHECK_FAILED))                                               /* SBSW_OS_CORE_COREISASRCORE_001 */
    {
    ...
    else
    {
      ...
      if(OS_UNLIKELY(localAsrCore != targetAsrCore))
      {
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(coreAsrDyn->UnhandledIrqReported != OS_UNHANDLEDEVENTREPORT_AVAILABLE))
    {
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_UnhandledExc,
(
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(coreAsrDyn->UnhandledExcReported != OS_UNHANDLEDEVENTREPORT_AVAILABLE))
    {
      status = OS_STATUS_STATE_1;
    }
    else
...
\ID SBSW_OS_FC_PRECONDITION_AND_CHECK
 \DESCRIPTION    The pointer passed to the called function is derived from an argument passed to the caller function.
                 ...
                 NULL_PTR, the pointer is correct. Preconditions are check during code review.
...
\ID SBSW_OS_FC_PRECONDITION
 \DESCRIPTION    The pointer passed to the called function is derived from an argument passed to the caller function.
                 ...
                 pointer is correct. Preconditions are check during code review.
...
\ID SBSW_OS_CORE_IOCINIT_001
 \DESCRIPTION    Os_IocInit is called with an entry of IocRefs of the systemCfg object. The systemCfg pointer
                 is derived from the return value of the function Os_SystemGet. The return value is always correct.
                 The used IocRefs index is limited to IocCount. IocRefs and IocCount is derived from a system object
                 given as argument.
 \COUNTERMEASURE \M [CM_OS_SYSTEM_IOCREFS_M]
...
\ID SBSW_OS_CORE_COREINTERRUPTHANDLINGINIT_001
 \DESCRIPTION    Os_CoreInterruptHandlingInit is called with the return value of Os_CoreId2Core.
 \COUNTERMEASURE \M [CM_OS_COREID2CORE_M]

\ID SBSW_OS_CORE_COREINTERRUPTHANDLINGINIT_002
...
\ID SBSW_OS_CORE_BARRIERSYNCHRONIZE_001
 \DESCRIPTION    Os_BarrierSynchronize is called with the Barrier object derived from the return value of
                 Os_ThreadGetCore.
 \COUNTERMEASURE \M [CM_OS_CORE_BARRIER_M]
                 \M [CM_OS_THREADGETCORE_M]
...
\ID SBSW_OS_CORE_APPSETNOTACCESIBLE_001
...
\ID SBSW_OS_CORE_APPSETNOTACCESIBLE_002
...
\ID SBSW_OS_CORE_HAL_COREEMULATEMASTER_001
 \DESCRIPTION    Os_Hal_CoreEmulateMaster is called with the HwConfig derived from the return value of
                 Os_CoreGetCurrentCore.
 \COUNTERMEASURE \S [CM_OS_COREGETCURRENTCORE_S]
                 \M [CM_OS_CORE_HWCONFIG_M]

\ID SBSW_OS_CORE_HAL_COREINITMASTER_001
 \DESCRIPTION    Os_Hal_CoreEmulateMaster is called with the HwConfig derived from the return value of
                 Os_SystemGet. Return value of Os_SystemGet is always correct.
 \COUNTERMEASURE \M [CM_OS_SYSTEM_HWCONFIG_M]

\ID SBSW_OS_CORE_HAL_COREEMULATESLAVE_001
 \DESCRIPTION    Os_Hal_CoreEmulateSlave is called with the HwConfig derived from the return value of
                 Os_CoreGetCurrentCore.
 \COUNTERMEASURE \S [CM_OS_COREGETCURRENTCORE_S]
                 \M [CM_OS_CORE_HWCONFIG_M]
...
\CM CM_OS_COREASR_STACKREFS_M
      Verify that:
        ...
        3. each element in StackRefs is no NULL_PTR.
...
\CM CM_OS_COREASR_INTERRUPTSOURCEREFS_M
      Verify that:
        ...
        3. each element in InterruptSourceRefs is no NULL_PTR.
...
\CM CM_OS_COREASR_RESOURCEREFS_M
      Verify that:
        ...
        3. each element in ResourceRefs is no NULL_PTR.
...
\CM CM_OS_COREASR_IOCREFS_M
      Verify that:
        ...
        3. each element in IocRefs is no NULL_PTR.
...
\CM CM_OS_COREASR_APPREFS_M
      Verify that:
        ...
        3. each element in AppRefs is no NULL_PTR.
...
\CM CM_OS_COREASR_HOOKREFS_M
      Verify that:
        ...
        3. each element in HookRefs is no NULL_PTR.
...
\CM CM_OS_SYSTEM_SPINLOCKREFS_M
      Verify that:
        ...
        3. each element in SpinlockRefs is no NULL_PTR.

\CM CM_OS_SYSTEM_IOCREFS_M
      Verify that:
        1. each system's IocRefs pointer is no NULL_PTR,
        2. each system's IocRefs size is equal to the systems's IocCount and
        3. each element in IocRefs is no NULL_PTR.
...
\ID COV_OS_UNSUPPORTED88830
   ...
   \ACCEPT TX
   \ACCEPT XF
   \REASON Feature is not yet supported, ESCAN prevents that the code becomes active. Unit test and code inspection
           have not found any risk in keeping this code. ESCAN00088830: Provide more rigid MP settings on start.

\ID COV_OS_UNSUPPORTED88879
   \ACCEPT XX
   \ACCEPT TX
   \ACCEPT XF
   \REASON Feature is not yet supported, generator prevents that the code becomes active. Unit test and code inspection
           have not found any risk in keeping this code. ESCAN00088879: Generator shall provide further variants
           of stack-sharing.
...
\ID COV_OS_UNSUPPORTED89701
   ...
   \ACCEPT TX
   \REASON Feature is not yet supported, ESCAN prevents that the code becomes active. Unit test and code inspection
           have not found any risk in keeping this code. ESCAN00089701: Add the ability to execute trusted
           applications in non privileged mode.
