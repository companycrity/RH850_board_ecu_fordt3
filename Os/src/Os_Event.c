 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Events  The event management object to modify.
 ...
 *  \details        --no details--
 *
 *  \param[out]     Events  The event management object to query.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Events  The event management object to query.
 *
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(OS_LIKELY(Os_TaskIsCoreLocal(task) != 0))                                                                      /* SBSW_OS_EV_TASKISCORELOCAL_001 */
      {
...
)
{
  ...
  else
  {
    ...
    else if(OS_UNLIKELY(Os_ThreadCheckSpinlocksReleased(currentThread) == OS_CHECK_FAILED))                             /* SBSW_OS_EV_THREADCHECKSPINLOCKSRELEASED_001 */
    {
    ...
    else
    {
      ...
      if((Os_EventWaitEventTriggered(event) != 0))                                                                      /* SBSW_OS_EV_EVENTWAITEVENTTRIGGERED_001 */
      {
        ...
        if(OS_LIKELY(Os_TpCheckInterArrival(configTp, threadTp) != OS_CHECK_FAILED))                                    /* SBSW_OS_TASK_TPCHECKINTERARRIVAL_001 */
        {
          ...
          Os_TpResetAndResume(configTp, threadTp, currentThread);                                                       /* SBSW_OS_TASK_TPRESETANDRESUME_001 */
...
)
{
  ...
  if(OS_LIKELY(callerCore == targetCore))
  {
...
\ID SBSW_OS_EV_TASKCONTINUERUNNINGTASK_001
 \DESCRIPTION    Os_TaskContinueRunningTask is called with the return value of Os_TaskThread2Task.
 \COUNTERMEASURE \R [CM_OS_TASKTHREAD2TASK_R]
