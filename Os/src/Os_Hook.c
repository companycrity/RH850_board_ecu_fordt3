 ...
 *  \param[in]    HookToBeNested     Hook to be nested.
 *
 ...
 *  \synchronous    TRUE
 *
 *  \pre            Given hook is a StatusHook.
 **********************************************************************************************************************/
 ...
 *  \synchronous    TRUE
 *
 *  \pre            Given hook is a ProtectionHook.
 **********************************************************************************************************************/
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_HookReturn, (void))
{
  ...
  /* #30 Call the return function depending on the hook configuration. */
  switch(currentHook->TaskReturn)
  {
    ...
    default:                                                                                                            /* COV_OS_INVSTATE */
      /* #40 For unknown values call the Panic Hook. */
      Os_ErrKernelPanic();
    break;
  }
...
))
{
  ...
  else
  {
    ...
    Os_ThreadCleanupAndResume(previousThread);                                                                          /* SBSW_OS_HK_THREADCLEANUPANDRESUME_001 */
  }
}
...
))
{
  ...
  Os_ThreadSuspendAndStart(currentThread, &(Hook->Thread));                                                             /* SBSW_OS_HK_THREADSUSPENDANDSTART_001 */
...
)
{
  ...
  else
  {
    ...
    hookDyn->HookTypeFlag = OS_HOOKFLAG_NOHOOKS;                                                                        /* SBSW_OS_FC_PRECONDITION */ /* SBSW_OS_HK_HOOKGETDYN_002 */

    /* #60 Bypass mode parameter.                 */
    hookDyn->Parameter.Mode = Mode;                                                                                     /* SBSW_OS_FC_PRECONDITION */ /* SBSW_OS_HK_HOOKGETDYN_002 */

    /* #70 Start hook thread. */
    Os_ThreadStartFirst(hookThread);                                                                                    /* SBSW_OS_HK_THREADSTARTFIRST_001 */
  }
}
...
)
{
  ...
  /*! \note Access not guaranteed, if MemoryProtectionForTrusted is enabled. */
...
FUNC(void, OS_CODE) Os_HookCallPanicHook(void)
{
  ...
  /*! \note Check may be removed. In case that no panic is provided, calling the internal function is not performance
   *        critical.  */
...
\CM CM_OS_HOOKTHREAD2HOOK_M
      Verify that the thread context of each Hook refers to Os_HookReturn and that there is
      no other reference to Os_HookReturn.
