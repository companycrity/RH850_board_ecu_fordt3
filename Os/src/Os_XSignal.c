 ...
 *  \details      --no details--
 *
 *  \param[in]    XSignal     The XSignal to query.
 ...
 *
 *  \param[in]    XSignal     The XSignal to query.
 *  \param[in]    IsrIdx      The index of the ISR.
 *
 ...
 *  \details      --no details--
 *
 *  \param[in]    SendThread  The XSignal sender thread.
 *  \param[in]    RecvCore    The XSignal receiver core.
 *
 *  \return       The send port which is connected to the given receiver core.
 *  \retval       NULL_PTR    If there is no connection to the requested core.
 *
 ...
 *  \details      --no details--
 *
 *  \param[in]    XSigIsr           XSignal to query.
 ...
 *
 *  \param[in,out]  SendPort      The send port to write to.
 *  \param[in]      SendThread    The sender of the cross core request.
 *  \param[in]      FunctionIdx   The function to call.
 *  \param[in]      Parameters    The parameters which are passed to the function.
 ...
 *  \details      --no details--
 *
 *  \param[in]    SendChannel    The send buffer that's results are handled.
 *
 *  \context      OS_INTERNAL
 *
 *  \reentrant    TRUE
 ...
 *  \details      --no details--
 *
 *  \param[in,out]    RecvPort    The receive port to initialize.
 *
 *  \context      OS_INTERNAL
 *
 *  \reentrant    TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  XSigIsr    The ISR to initialize.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in]      Thread  Thread which shall be converted.
 *
 *  \context        ISR2
 *
 *  \reentrant      TRUE
 ...
 *  Os_XSigFunctionIdx2SerciceId()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE OSServiceIdType, OS_CODE, OS_ALWAYS_INLINE, Os_XSigFunctionIdx2SerciceId,
(
  Os_XSigFunctionIdx FunctionIdx
))
{
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_XSigHandleResults,
(
  P2CONST(Os_XSigChannelSendConfigType, TYPEDEF, OS_CONST) SendChannel
))
{
  ...
  while(Os_XSigChannelIsResultAvailable(SendChannel) != 0)                                                              /* SBSW_OS_FC_PRECONDITION */
  {
    ...
    if(OS_LIKELY(comData->SendData->Acknowledge == OS_XSIGSTATE_ASYNC_NACK))
    {
      ...
      {
        /*! \note Discuss: Switch to the timing protection of the sender? */
        ...
        /*! \note This code uses error module internal knowledge. */
        ...
        /*! \note Introduce reporting function for XSignal and use it here. */
 ...
 *  Os_XSigThread2XSigIsr()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE P2CONST(Os_XSigIsrConfigType, AUTOMATIC, OS_CONST), OS_CODE,
OS_ALWAYS_INLINE, Os_XSigThread2XSigIsr,
(
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) Thread
))
...
ISR(Os_XSigRecvIsrHandler)
{
  ...
  do
  {
    ...
    for(it = 0; it < xSigIsr->RecvPortCount; ++it)
    {
      ...
      /* #40 If the core has send a request: */
      if(Os_XSigChannelIsReceiveElementAvailable(&(recvPort->RecvChannel)) != 0)                                        /* SBSW_OS_XSIG_XSIGCHANNELISRECEIVEELEMENTAVAILABLE_001 */
      {
        ...
        /* #50 Check that the function is available. */
        ...
        /* #60 Call the respective function. */
        else
        {
    ...
    /* #80 While any buffer had entries. */
...
)
{
  ...
  if(Os_XSigIsEnabled() != 0)
  {
    ...
    /*! \note What about non nestable ISRs. Deadlocks possible? */
    ...
    else
    {
      ...
      if(status == OS_STATUS_OK)
      {
        ...
        {
          /*! \note Interrupts are expected to be enabled, in order to handled incoming requests. This limitation can be
           *        faced, by polling for incoming requests with interrupts disabled.  */
          Os_Hal_CoreNop();
        }
