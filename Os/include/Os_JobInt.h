 ...
 *
 *  \param[in]      LeftJob     The left job.
 *  \param[in]      RightJob    The right job.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  TargetJob     The job who's expiry time shall be set.
 *  \param[in]      SourceJob     The job who's expiry time shall be read.
 *
 *  \context        ANY
 *
 *  \reentrant      TRUE
 ...
 *  \details        --no details--
 *
 *  \param[in]      Job       The job to modify.
 *  \param[in]      TimeStamp The new expiration time stamp of the job.
 *
 *
 *  \context        ANY
 *
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            ExpirationTime is in range 0 <= ExpirationTime <= MaxCountingValue.
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 *  \pre            Counter belongs to the local core.
 *  \pre            Given value of Start is in range 1..MaxAllowedValue of the respective counter configuration.
 *  \pre            Given job is not scheduled.
 **********************************************************************************************************************/
 ...
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Concurrent access to given object is prevented by caller.
 *  \pre            TimeStamp has to be in range 0 <= TimeStamp <= MaxCountingValue.
 **********************************************************************************************************************/
 ...
 *
 *  \param[in]      Job       The job.
 *  \param[in]      Reduce    The value to subtract from the MaxAllowedValue.
 *  \param[in]      Value     The value to check.
 *
 *  \return         Whether the check succeeded or not.
 *  \retval         Not OS_CHECK_FAILED   If Value exceeds the limit.
 ...
 *
 *  \pre            Given object pointer(s) are valid.
 *  \pre            Parameter Reduce <= MAXALLOWEDVALUE of the underlying counter
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE,
Os_JobCheckValueLeReducedMaxAllowed,
(
 ...
 *  \details        --no details--
 *
 *  \param[in]      Job_1  The first job to check.
 *  \param[in]      Job_2  The second job to check.
 *
 *  \retval         Not OS_CHECK_FAILED   Both job are assigned to the same counter.
 *  \retval         OS_CHECK_FAILED       The sjobs are assigned to different counters.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
