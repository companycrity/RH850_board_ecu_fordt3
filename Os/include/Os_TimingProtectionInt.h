 ...
 *  \details        --no details--
 *
 *  \param[in]      TpThread    The TP thread to query.
 *
 ...
 *  \details        --no details--
 *
 *  \param[in]      Config    The timing protection configuration to query.
 *
 *  \return         Pointer to a monitored thread.
 *
 *  \context        ANY
 ...
 *  \synchronous  TRUE
 *
 *  \pre          Given object pointer(s) are valid, if timing protection is enabled.
 ...
 *  \param[in]      RemainingTime     The time of the next expiration relative to now.
 *
 *  \context      OS_INTERNAL
 *
 *  \reentrant    TRUE for different objects.
 ...
 *  \pre          Given object pointer(s) are valid.
 *  \pre          Value has to be in range 0 <= Value <= MaxCountingValue.
 *  \pre          Value has to be in range 0 <= Add   <= MaxAllowedValue.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE,
Os_TpAdd,
(
 ...
 *  \details
 *  Stops and resets the current execution budget. So when the budget is resumed the next time, the full time
 *  is available again. Resumes the execution and lock budget. Reprogram hardware timer.
 *
 *  \param[in,out]  Config      The timing protection configuration of the current core.
 *  \param[in,out]  Next        Budgets of next thread.
 *  \param[in]      NextThread  The next thread to monitor.
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Current     Budgets of the current thread.
 ...
 *  \pre          Given object pointer(s) are valid, if timing protection is enabled.
 *  \pre          Concurrent access to given object is prevented by caller.
 *  \pre          Resource ID is in a valid range.
 *
 ...
 *  \param[in]      Resource  The ID of the resource.
 *
 *  \context      OS_INTERNAL
 *
 *  \reentrant    TRUE
